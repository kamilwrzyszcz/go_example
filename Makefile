...
createdb:
	docker exec -it example_postgres createdb --username=root --owner=root example_postgres
create_testdb:
	docker exec -it example_postgres createdb --username=root --owner=root test_postgres
mock_db:
...
stop_redis:
	docker stop redis
dropdb:
	docker exec -it example_postgres dropdb -example_postgres
drop_testdb:
	docker exec -it example_postgres dropdb -test_postgres
migrateup:
	migrate -path db/migration -database "postgresql://root:secret@localhost:5432/example_postgres?sslmode=disable" -verbose up
migratedown:
